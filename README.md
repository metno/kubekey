...
aquire an ID Token from the OIDC provider and pass it to the kubernetes API.

See discussion about why kube-apiserver supports [ID Token instead of Access
Token](https://discuss.kubernetes.io/t/why-use-id-tokens-instead-of-access-tokens-for-authorization/27105/3),
which is normally [discouraged](https://auth0.com/blog/id-token-access-token-what-is-the-difference/).
TLDR; ID Token is guaranteed to be a JWT.
...
3. kubekey checks with the [operating system keyring](https://en.wikipedia.org/wiki/GNOME_Keyring) if kubekey has cached a non-expired ID Token, if it has, it provides the token immedeately (go to step 10)
...
9. The browser redirects to kubekey's short lived local http server with the OIDC access token, kubekey requests an ID Token and caches that in the operating system keyring
10. kubekey provides kubectl/client-go with the ID token
11. kubectl/client-go sends API-requests to kube-apiserver with the ID token. kube-apiserver then validates the signature and extracts information about the user
...
4. Configure your kubernetes cluster to trust an OIDC issuer, see https://kubernetes.io/docs/reference/access-authn-authz/authentication/#configuring-the-api-server
  * [--oidc-client-id](https://kubernetes.io/docs/reference/command-line-tools-reference/kube-apiserver/#:~:text=%2D%2Doidc%2Dclient%2Did%20string)
  * [--oidc-issuer-url](https://kubernetes.io/docs/reference/command-line-tools-reference/kube-apiserver/#:~:text=%2D%2Doidc%2Dissuer%2Durl%20string)
...
Copyright (C) 2019 - 2026 MET Norway. kubekey is licensed under [GPL version 2](https://github.com/metno/kubekey/blob/master/LICENSE) or (at your option) any later version.
